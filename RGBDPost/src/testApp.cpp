...
void testApp::setup(){
    ...
    currentKinectCheckers.allocate(640,480, OF_IMAGE_COLOR);
    currentExternalCheckers.allocate(640,480, OF_IMAGE_COLOR);

	//**load source wigdet
 	loadSource = new scrGroupGrid("Load Source");
    clipWidgets = new scrWidgets();
	pointcloudTimeline = new scrDraw2D();
	externalTimeline = new scrDraw2D();

    calibrate = new scrGroupGrid("Calibrate");
	calibrateWidgets = new scrWidgets();
	externalCheckerView = new scrDraw2D();
	kinectCheckerView = new scrDraw2D();
	pointcloudView = new scrDraw3D();

    mainScreen = new scrGroupTabbed("", 32);
    mainScreen->push(loadSource);
